...
The id is the **whole stem**, not the date. Two records on one day is normal, and an id that names
two decisions is the failure this ledger exists to prevent. Two authors collide only on the same
date *and* the same slug, and even that arrives as an add/add conflict on a filename, which git
reports loudly.
