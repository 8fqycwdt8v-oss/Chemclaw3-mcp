...
def test_egress_is_denied() -> None:
    ...
    assert spec["egress"] == [], f"suitability must reach nothing; found {spec['egress']!r}"
...
def test_ingress_admits_only_the_agent_and_the_scraper() -> None:
    ...
    ports = rules[0]["ports"]
    assert ports == [{"protocol": "TCP", "port": 8892}]
...
def test_the_container_declares_the_port_the_manifest_dials() -> None:
    ...
    assert "8892" in containerfile
    assert ":8892/mcp" in manifest["endpoint"]["url"]
...
def test_the_scrape_is_wired_end_to_end() -> None:
    ...
    assert service["spec"]["selector"]["app.kubernetes.io/name"] == "chemclaw-mcp-suitability"
    ports = service["spec"]["ports"]
    assert ports == [{"name": "http", "protocol": "TCP", "port": 8892, "targetPort": 8892}]
