...
is written in *both* directions, the way Chemclaw3's `test_deploy_chart.py` pins its chart, because
the realistic regression is not someone adding an egress rule on purpose. It is someone dropping
`Egress` from `policyTypes` while debugging and leaving the file looking unchanged.
"""
...
def test_egress_is_denied() -> None:
    ...
    assert spec["egress"] == [], f"kinetics must reach nothing; found {spec['egress']!r}"
...
def test_ingress_admits_only_the_agent_and_the_scraper() -> None:
    ...
    ports = rules[0]["ports"]
    assert ports == [{"protocol": "TCP", "port": 8852}]
...
def test_the_policy_selects_this_server() -> None:
    ...
    spec = _policy()["spec"]
    assert isinstance(spec, dict)
    assert spec["podSelector"]["matchLabels"]["app.kubernetes.io/name"] == ("chemclaw-mcp-kinetics")
...
def test_the_container_declares_the_port_the_manifest_dials() -> None:
    ...
    manifest = yaml.safe_load(
        (POLICY.parents[1] / "connector.yaml").read_text(encoding="utf-8"),
    )
    assert "8852" in containerfile
    assert ":8852/mcp" in manifest["endpoint"]["url"]
...
def test_the_scrape_is_wired_end_to_end() -> None:
    ...
    assert service["spec"]["selector"]["app.kubernetes.io/name"] == "chemclaw-mcp-kinetics"
    ports = service["spec"]["ports"]
    assert ports == [{"name": "http", "protocol": "TCP", "port": 8852, "targetPort": 8852}]
    assert monitor["spec"]["selector"]["matchLabels"]["app.kubernetes.io/name"] == (
        "chemclaw-mcp-kinetics"
    )
